# Swift Site — Astro + Vite Clone

A clone of swift.org built with [Astro](https://astro.build), deployable to **Cloudflare Pages** and **GitHub Pages**.
...
3. Create the Pages project name first (one time):
   ```bash
   npx wrangler pages project create swift-site
   ```
...
# Deploy
wrangler pages deploy dist --project-name=swift-site
```
...
3. Set your repo name in `astro.config.mjs`:
   ```js
   const repoName = "swift-site"; // ← change to your actual repo name
   ```
